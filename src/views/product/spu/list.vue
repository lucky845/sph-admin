    ...
    <div v-show="!showSpuForm && !showSkuForm">
      ...
      <el-table
        ...
        <el-table-column label="操作" width="200" align="center">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" icon="el-icon-plus" @click="addSku(scope.row.id, scope.row.productName, scope.row.brandId)">添加SKU</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    fetchData(page = 1) {
      ...
      spu.getPageList(this.page, this.limit, this.searchObj.category3Id).then(
        response => {
          //
    ...
    // 添加sku
    addSku(productId, productName, brandId) {
      this.selectedSpu.productId = productId
      this.selectedSpu.productName = productName

      // 初始化表单
      this.$refs.skuForm.init(productId, this.category1Id, this.category2Id, this.category3Id, brandId)
