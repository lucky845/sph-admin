<template>
  ...
  <el-form :model="spuForm" label-width="70px">
    ...
    <el-form-item label="品牌" prop="brandId">
      <el-select v-model="spuForm.brandId" placeholder="请选择">
        <el-option
          v-for="trademark in trademarkList"
          :key="trademark.id"
          :label="trademark.brandName"
          :value="trademark.id"/>
      </el-select>
    </el-form-item>
    ...
    <el-form-item label="图片上传" >
      <el-upload
        ...
        :multiple="true"
        :action="BASE_API+'/product/brand/fileUpload'"
        class="upload-demo"
    ...
    <div>
      <el-table
        ...
        <el-table-column label="销售属性值">
          <template slot-scope="scope">
            <el-tag
              v-for="tag in scope.row.salePropertyValueList"
              :key="tag"
...
export default {
  ...
  data() {
    return {
      ...
      spuSaleAttrListTemp: [], // 临时数据：格式：baseSaleAttrId|salePropertyKeyName
  ...
  methods: {
    ...
    saveSpuInfo() {
      ...
      this.spuForm.salePropertyKeyList = []
      this.spuSaleAttrListTemp.forEach(spuSaleAttr => {
        // 处理销售属性
        const arr = spuSaleAttr.saleAttr.split('|')
        spuSaleAttr.salePropertyKeyId = arr[0]
        spuSaleAttr.salePropertyKeyName = arr[1]
        const saleAttr = {
          salePropertyKeyId: arr[0],
          salePropertyKeyName: arr[1],
          salePropertyValueList: []
        }

        // 处理销售属性值
        spuSaleAttr.salePropertyValueList.forEach(spuSaleAttrValue => {
          const saleAttrValue = {
            salePropertyKeyId: arr[0],
            salePropertyValueName: spuSaleAttrValue
          }
          saleAttr.salePropertyValueList.push(saleAttrValue)
        })
        //------XXX-------
        this.spuForm.salePropertyKeyList.push(saleAttr)
      })
    ...
    onUploadSuccess(res, file) {
      ...
      // 填充上传文件列表
      this.spuForm.productImageList.push({
        imageName: file.name,
    ...
    addSaleAttr() {
      const saleAttr = {
        ...
        edit: false,
        salePropertyValueList: [] // [tag1, tag2, tag3]
      }
    ...
    deleteSaleAttr(saleAttr) {
      const tempList = []
      //--------
      this.spuForm.salePropertyKeyList.forEach(item => {
        if (item.saleAttr !== saleAttr) {
          tempList.push(item)
        }
      })
      this.spuForm.salePropertyKeyList = tempList
    },
    ...
    // 保存销售属性值
    saveAttrValue(row) {
      if (!row.salePropertyValueList) {
        row.salePropertyValueList = []
      }

      const saleAttrValue = row.saleAttrValue
      if (saleAttrValue) {
        row.salePropertyValueList.push(saleAttrValue)
      }
    ...
    handleTagClose(tag, row) {
      const index = row.salePropertyValueList.indexOf(tag)
      row.salePropertyValueList.splice(index, 1)
    }
  }
}
</script>
