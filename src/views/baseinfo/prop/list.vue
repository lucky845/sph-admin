  ...
  <div class="app-container">
    ...
    <div v-show="!showAttrInfoForm">
      ...
      <el-table
        ...
        <el-table-column label="操作" width="200" align="center">
          <template slot-scope="scope">
            <el-button v-if="categoryLevel == scope.row.categoryLevel" type="primary" size="mini" icon="el-icon-edit" @click="editAttrInfoById(scope.row.id, scope.row.propertyKey)">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-form v-show="showAttrInfoForm" :model="attrInfoForm" :inline="true" class="demo-form-inline">
      ...
      <el-form-item label="属性名称">
        <el-input v-model="attrInfoForm.propertyKey"/>
      </el-form-item>
      ...
      <div>
        <el-table
          ...
          :data="attrInfoForm.propertyValueList"
          element-loading-text="数据正在加载......"
          border
          fit
          highlight-current-row>
          ...
          <el-table-column label="属性值名称">
            ...
            <template slot-scope="scope">
              ...
              <span
                v-else
                @click="editAttrValue(scope.row)">{{ scope.row.propertyValue }}</span>
            </template>
          </el-table-column>

          <el-table-column label="操作" width="200" align="center">
            <template slot-scope="scope">
              <el-button type="danger" size="mini" icon="el-icon-edit" @click="deleteAttrValueByName(scope.row.propertyValue)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
export default {
  ...
  methods: {
    ...
    addAttrInfo() {
      ...
      this.attrInfoForm.propertyKey = null
      this.attrInfoForm.propertyValueList = []
    ...
    // 修改属性
    editAttrInfoById(propertyKeyId, propertyKey) {
      // 获取属性值列表
      this.attrValueListLoading = true
      prop.getAttrValueList(propertyKeyId).then(response => {
        this.attrInfoForm.id = propertyKeyId
        this.attrInfoForm.propertyKey = propertyKey
        this.attrInfoForm.propertyValueList = response.data
    ...
    deleteAttrValueByName(attrValueName) {
      const tempList = []
      this.attrInfoForm.propertyValueList.forEach(attrValue => {
        if (attrValue.propertyValue !== attrValueName) {
          tempList.push(attrValue)
        }
      })
      this.attrInfoForm.propertyValueList = tempList
    },
    ...
    addAttrValue() {
      ...
      this.attrInfoForm.propertyValueList.push(attrValue)
    },
